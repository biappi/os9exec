...
/* Get alias resolved FSSpec */
{
    ...
    if (pathname == NULL)
        ...
        strcpy(cstr, pathname); /* make a local copy */
    strcpy(rstr, "");

    debugprintf(dbgFiles,
                dbgNorm,
                ("# Resolved_FSSpec(Vol=%d, Dir=%ld, Name='%s')\n",
                 volID,
                 dirID,
                 cstr));
    ...
    while (true) {
        ...
        if (oserr == fnfErr && fsP->name[0] != 0 && fsP->vRefNum != 0 &&
            fsP->parID != 0) {
            ii = 0;
            do {
                fName[0]                        = 0;
        ...
        /*     strcpy( tmp,cstr );
               while ((oserr==fnfErr ||
                   ...
                   strcpy( pastmp,tmp );
                   c2pstr( pastmp ); oserr= FSMakeFSSpec( volID,dirID, pastmp,
           fsP ); if (!oserr) {
                   ...
                   if (oserr==0 ||
                       oserr==dirNFErr) oserr= FSMakeFSSpec( volID,dirID,
           pasmac, fsP ); } // while
       */
        ...
        if (!oserr) {
            ...
            if (oserr || /* an error is always bad */
                rstr[0] == NUL)
                break; /* no sub path to reconnect */
            ...
            if (*isFolder) {
                if (dirID == 1) { /* root dir does not start with ":" on Mac */
                    while (*p == ':')
                        p++; /* no up paths allowed for root */
                }
                else
                    strcpy(cstr, ":");
            }
    ...
    debugprintf(
        dbgFiles,
        dbgNorm,
        ("# Resolved_FSSpec(Vol=%d, Dir=%ld, Name='%s') returns oserr=%d\n",
         volID,
         dirID,
         cstr,
         oserr));
...
/* make FSSpec from mac path / exe flag */
{
    ...
    case _appl:
        ...
        break;
    case _root:
        volID = procs[1].d.volID;
        dirID = procs[1].d.dirID;
        break;
    case _rootX:
        volID = procs[1].x.volID;
        dirID = procs[1].x.dirID;
        break;
    case _data:
        volID = procs[pid].d.volID;
        dirID = procs[pid].d.dirID;
        break;
    case _exe:
        volID = procs[pid].x.volID;
        dirID = procs[pid].x.dirID;
        break;
    case _mdir:
        volID = mdir.volID;
        dirID = mdir.dirID;
...
/* get ID of directory */
{
    ...
    debugprintf(
        dbgFiles,
        dbgDetail,
        ("# get_dirid: MakeFSSpec(Vol=%d, Dir=%ld, Name='%s') returns err=%d\n",
         *volID_P,
         *dirID_P,
         pathname == NULL ? "<NULL>" : pathname,
         err));
    ...
    if (!(cipb.hFileInfo.ioFlAttrib & ioDirMask))
        return os9error(E_FNA); /* it's a file, return error */
    ...
    *volID_P = 0; // don't use it directly for this case
    ...
    if (*volID_P == 0 && fdID < MAXDIRS) { // if the range is correct
        ...
        if (mP && mP->ident != NULL) {
            a = mP->ident;
            b = strstr(a, " ");
            b++; // abd divide it into its pieces
