 ...
 *                 - vector=TRAP1..15 vector, func=TCALL-function, stack ready
 * for calling trap handler routine. OS9 regs must be set up (A6, PC).
 *                 - vector=$FAFA, func=error trap vector offset, stack ready
 * for F$STrap service routine. OS9 regs (except pc,a6 and eventually A7)
 * prepared for service routine call, too.
 */
...
/* everyone uses it, but no 68k library provides it (but MW C Tutorial 2 68k
 * does!) */
static Boolean TrapAvailable(short tNum)
{
    if (NGetTrapAddress(tNum, OSTrap) != GetToolTrapAddress(_Unimplemented))
        return true;
    return NGetTrapAddress(tNum, ToolTrap) !=
           GetToolTrapAddress(_Unimplemented);
    // MW   return ( NGetTrapAddress(tNum,tType) !=
...
{
#if GENERATINGCFM
    return CallUniversalProc((UniversalProcPtr)(*llmhandle),
                             uppOS9goProcInfo,
                             SEL_OS9GO,
                             rp);
#else
...
{
#if GENERATINGCFM
    CallUniversalProc((UniversalProcPtr)(*llmhandle),
                      uppCacheRFProcInfo,
                      SEL_CACHERF,
                      addr,
                      size);
...
#if GENERATINGCFM
    if (llmhandle == NULL) {
        upho_printf("llm_os9_debug (PPC): low-level not prepared, can't go to "
                    "debugger\n");
    }
    else {
        CallUniversalProc((UniversalProcPtr)(*llmhandle),
...
void Flush68kCodeRange(void *address, ulong size)
{
    if (needsflush) {
        ...
        if (fullcache) {
            FlushCodeCacheRange(
                address,
                size); /* make sure llm code is safely installed */
            /* data cache */
...
OSErr lowlevel_prepare(void)
{
    ...
    if (Gestalt(gestaltProcessorType, &gestaltresponse) ==
        0) { /* get 680xx processor type */
        needsflush =
            (gestaltresponse >=
             gestalt68020); /* 68020 and above have an instruction chache */
    }
    else
        needsflush = false; /* no gestalt, no cache */
    /* FPU */
    if (Gestalt(gestaltFPUType, &gestaltresponse) ==
        0) {                                        /* get 68xxx FPU type */
        hasfpu = (gestaltresponse != gestaltNoFPU); /* has some kind of FPU */
    }
    else
        hasfpu = false; /* no gestalt, no FPU */

/* cache services */
#ifndef TRAPAVAIL_UNAVAILABLE
    fullcache = (TrapAvailable(
        _HWPriv)); /* check if we have all the Cache control calls */
#else
    ...
    if (Gestalt(gestaltVMAttr, &gestaltresponse) == 0) { /* get VM attribute */
        vmenabled =
            (gestaltresponse & (1 << gestaltVMPresent)); /* set if VM is on */
    }
    else
        vmenabled = false; /* no gestalt, no virtual memory */
    ...
    /* --- make the low-level code safe */
    {
        ...
        Flush68kCodeRange(*llmhandle,
                          size); /* make sure llm code is safely installed */
        debugprintf(dbgStartup,
                    dbgDeep,
                    ("# lowlevel_prepare: (PPC) 'O9LM' loaded at $%08lX\n",
                     *llmhandle));
    }
    /* test user mode */
    usesusermode = CallUniversalProc((UniversalProcPtr)(*llmhandle),
                                     uppUMProbeProcInfo,
                                     SEL_UMPROBE);
#else
    ...
    debugprintf(
        dbgStartup,
        dbgDeep,
        ("# lowlevel_prepare: (68k) low level code is static linked\n"));
    /* test user mode */
    ...
    if (usesusermode) {
        debugprintf(dbgStartup,
                    dbgDeep,
                    ("# lowlevel_prepare: (68k) 68k-low-level code cannot yet "
                     "handle Mac program running in user mode\n"));
        uphe_printf("FATAL ERROR: 'os968k' cannot run on this PowerMac / on "
                    "this operating system\n");
        uphe_printf("              Please use the PowerPC-native 'os9ppc'\n");
        uphe_printf(
            "                                 or the FAT 'os9' tool instead\n");
        uphe_printf("\n");
    ...
    debugprintf(dbgStartup,
                dbgDeep,
                ("# lowlevel_prepare: vmenabled=%d, needsflush=%d, "
                 "fullcache=%d, hasfpu=%d, usesusermode=%d\n",
                 vmenabled,
                 needsflush,
                 fullcache,
                 hasfpu,
