    ...
    if (debugcheck(dbgWarnings, dbgNorm)) {
        if (mode & RCHK_DRU) {
            ...
            if ((reg & 0xFFFFFFF0) == 0xDDDDDDD0) {
                problem = true;
                uphe_printf("regcheck: %s = $%08lX seems to be uninitialized "
                            "data reg (pid=%d)\n",
                            nam,
                            reg,
                            pid);
        ...
        if (mode & (RCHK_MEM + RCHK_MOD)) {
            ...
            problem = true;
            if (RCHK_MEM && !out_of_mem(pid, reg)) {
                problem = false;
                goto ok;
            }
            if (RCHK_MOD && !out_of_mods(reg)) {
                problem = false;
                goto ok;
            }
        ok:
            if (problem)
                uphe_printf("regcheck: %s = $%08lX (pid=%d) is out of: %s %s\n",
                            nam,
                            reg,
                            pid,
                            mode & RCHK_MEM ? "[allocated memory]" : "",
                            mode & RCHK_MOD ? "[all OS9 modules]" : "");
        }
        if (problem) {
...
/* display debug help, allow internal and external access */
{
    ...
    upho_printf("\n");
    upho_printf("  dbgAnomaly      0x0001  anomalies: exceptions and "
                "unimplemented/dummy system calls\n");
    upho_printf(
        "  dbgSysCall      0x0002  call and return status of system calls\n");
    ...
    upho_printf("  dbgStartup      0x0800  os9exec/nt startup and initial "
                "argument preparation\n");
    upho_printf("  dbgPartial      0x1000  warnings for inaccurate/partial "
                "emulated functions\n");
    upho_printf(
        "  dbgTerminal     0x2000  interactive I/O, output filtering\n");
    upho_printf(
        "  dbgSpecialIO    0x4000  special I/O, such as printer and network\n");
...
#define INPLEN 100
    ...
    do {
        uphe_printf("Pid=%d%s: dbgmsk=$%04X,$%04X,$%04X stop=$%04X "
                    "trigger='%s' (type ?<Enter> for hlp)\n",
                    currentpid,
                    currentpid >= MAXPROCESSES ? " **NONE**" : "",
                    debug[0],
                    debug[1],
                    debug[2],
                    debughalt,
        ...
        *cp = NUL; /* string termination */
        if (cp == inp)
            continue;
#else
        if (fgets(inp, INPLEN, stdin) == NULL)
            continue;
#endif
        ...
        case 'r':
            if (tolower(inp[1]) == 'u')
                temp = MAXPROCESSES;
            else if (sscanf(&inp[1], "%hd", &temp) < 1)
                temp = currentpid;
            dumpregs(temp);
            break;
        ...
        case 'd':
            numitems = sscanf(&inp[1], "%hx,%hx", &temp, &temp2);
            if (numitems < 1) {
        ...
        case 's':
            if (sscanf(&inp[1], "%hx", &debughalt) < 1) {
        ...
        case 'e':
            m68k_dumpstate((uaecptr *)&listbase, false);
            break;
        case 't':
            m68k_os9trace = true;
        ...
        case 'o':
            if (sscanf(&inp[1], "%d", &dbgOut) < 1) {
                dbgOut = 0;
            };
            break;
        case 'w':
            if (sscanf(&inp[1], "%d", &without_pid) < 1) {
                without_pid = 0;
            };
            break;
        case 'j':
            if (sscanf(&inp[1], "%d", &justthis_pid) < 1) {
                justthis_pid = 0;
            };
            break;
        ...
        default:
            upe_printf(
                "[X]tra-[G]o, [K[xx]] Kill proc[xx], [Q]uit emulation\n");
            ...
            upe_printf("[M]odules, [P]rocs, [F[xx]]iles [of pid=xx], [V[xx]] "
                       "memory [of pid=xx]\n");
...
void get_error_strings(os9err err, char **nameP, char **descP)
{
    ...
    case E_NOTNUM:
        name = "E_NOTNUM";
        desc = "number not found/not a number";
        break;
    case E_ILLARG:
        name = "E_ILLARG";
    ...
    case E_BUSERR:
        name = "E_BUSERR";
        desc = "bus error TRAP 2 occurred";
        break;
    case E_ADRERR:
        name = "E_ADRERR";
        desc = "address error TRAP 3 occurred";
        break;
    case E_ILLINS:
        name = "E_ILLINS";
        desc = "illegal instruction TRAP 4 occurred";
        break;
    case E_ZERDIV:
        name = "E_ZERDIV";
        desc = "zero divide TRAP 5 occurred";
        break;
    case E_CHK:
        name = "E_CHK";
        desc = "CHK instruction TRAP 6 occurred";
        break;
    case E_TRAPV:
        name = "E_TRAPV";
        desc = "TrapV instruction TRAP 7 occurred";
        break;
    case E_VIOLAT:
        name = "E_VIOLAT";
        desc = "privelage violation TRAP 8 occurred";
        break;
    case E_TRACE:
        name = "E_TRACE";
        desc = "Uninitialized Trace TRAP 9 occurred";
        break;
    case E_1010:
        name = "E_1010";
        desc = "Uninitialized 1010 TRAP 10 occurred";
        break;
    case E_1111:
        name = "E_1111";
        desc = "Uninitialized 1111 TRAP 11 occurred";
        break;
    case E_TRAP:
        name = "E_TRAP";
        desc = "user traps 1-15 E_TRAP+n";
        break;
    case E_FPUNORDC:
        name = "E_FPUNORDC";
        desc = "FP unordered condition error";
        break;
    case E_FPINXACT:
        name = "E_FPINXACT";
        desc = "FP inexact result";
        break;
    case E_FPDIVZER:
        name = "E_FPDIVZER";
        desc = "FP zero divisor";
        break;
    case E_FPUNDRFL:
        name = "E_FPUNDRFL";
        desc = "FP underflow";
    ...
    case E_FPNOTNUM:
        name = "E_FPNOTNUM";
        desc = "FP not a number";
        break;

    case E_PERMIT:
        name = "E_PERMIT";
        desc = "You must be super user to do that";
        break;
    case E_DIFFER:
        name = "E_DIFFER";
        desc = "Arguments to F$ChkNam are different";
        break;
    case E_STKOVF:
        name = "E_STKOVF";
        desc = "Stack overflow (ChkNam)";
        break;
    case E_EVNTID:
        name = "E_EVNTID";
        desc = "Invalid or Illegal event ID number ";
        break;
    case E_EVNF:
        name = "E_EVNF";
        desc = "Event (name) not found";
        break;
    case E_EVBUSY:
        name = "E_EVBUSY";
        desc = "The event is busy (can't be deleted) ";
        break;
    case E_EVPARM:
        name = "E_EVPARM";
        desc = "Impossible Event parameters supplied ";
        break;
    case E_DAMAGE:
        name = "E_DAMAGE";
        desc = "System data structures are damaged ";
        break;
    case E_BADREV:
        name = "E_BADREV";
        desc = "Incompatible revision";
        break;
    case E_PTHLOST:
        name = "E_PTHLOST";
        desc = "Path Lost (net node was down)";
        break;
    case E_BADPART:
        name = "E_BADPART";
        desc = "Bad partition data or no active";
        break;
    case E_HARDWARE:
        name = "E_HARDWARE";
        desc = "Hardware is damaged";
        break;

    case E_PTHFUL:
        name = "E_PTHFUL";
        desc = "Path Table full";
        break;
    case E_BPNUM:
        name = "E_BPNUM";
        desc = "Bad Path Number";
        break;
    case E_POLL:
        name = "E_POLL";
        desc = "Polling Table Full";
        break;
    case E_BMODE:
        name = "E_BMODE";
        desc = "Bad Mode";
    ...
    case E_DIRFUL:
        name = "E_DIRFUL";
        desc = "Module Directory Full";
        break;
    case E_MEMFUL:
        name = "E_MEMFUL";
        desc = "Process Memory Full";
        break;
    case E_UNKSVC:
        name = "E_UNKSVC";
        desc = "Unknown Service Code";
        break;
    case E_MODBSY:
        name = "E_MODBSY";
        desc = "Module Busy";
    ...
    case E_VCTBSY:
        name = "E_VCTBSY";
        desc = "IRQ Vector is already used";
        break;
    case E_NES:
        name = "E_NES";
        desc = "Non-Existing Segment";
        break;
    case E_FNA:
        name = "E_FNA";
        desc = "File Not Accessible";
        break;
    case E_BPNAM:
        name = "E_BPNAM";
        desc = "Bad Path Name";
    ...
    case E_PNNF:
        name = "E_PNNF";
        desc = "Path Name Not Found";
        break;
    case E_SLF:
        name = "E_SLF";
        desc = "Segment List Full";
        break;
    case E_CEF:
        name = "E_CEF";
        desc = "Creating Existing File";
        break;
    case E_IBA:
        name = "E_IBA";
        desc = "Illegal Block Address";
        break;
    case E_HANGUP:
        name = "E_HANGUP";
        desc = "Phone hangup occurred (modem)";
        break;
    case E_MNF:
        name = "E_MNF";
    ...
    case E_NOCLK:
        name = "E_NOCLK";
        desc = "System has no clock running";
        break;
    case E_DELSP:
        name = "E_DELSP";
        desc = "Deleting Stack Pointer memory";
        break;
    case E_IPRCID:
        name = "E_IPRCID";
        desc = "Illegal Process ID";
        break;
    case E_PARAM:
        name = "E_PARAM";
        desc = "Impossible polling parameter specified";
        break;
    case E_NOCHLD:
        name = "E_NOCHLD";
        desc = "No Children";
    ...
    case E_ITRAP:
        name = "E_ITRAP";
        desc = "Invalid or unavailable Trap code";
        break;
    case E_PRCABT:
        name = "E_PRCABT";
        desc = "Process Aborted";
        break;
    case E_PRCFUL:
        name = "E_PRCFUL";
        desc = "Process Table Full";
        break;
    case E_IFORKP:
        name = "E_IFORKP";
        desc = "Illegal Fork Parameter";
        break;
    case E_KWNMOD:
        name = "E_KWNMOD";
        desc = "Known Module";
    ...
    case E_SIGNAL:
        name = "E_SIGNAL";
        desc = "Signal Error (replaces E_USIGP)";
        break;
    case E_NEMOD:
        name = "E_NEMOD";
        desc = "Non Existing Module";
        break;
    case E_BNAM:
        name = "E_BNAM";
        desc = "Bad Name";
    ...
    case E_BMHP:
        name = "E_BMHP";
        desc = "bad module header parity";
        break;
    case E_NORAM:
        name = "E_NORAM";
        desc = "No Ram Available";
        break;
    case E_DNE:
        name = "E_DNE";
    ...
    case E_NOTASK:
        name = "E_NOTASK";
        desc = "No available Task number";
        break;
    case E_UNIT:
        name = "E_UNIT";
        desc = "Illegal Unit (drive)";
        break;
    case E_SECT:
        name = "E_SECT";
        desc = "Bad SECTor number";
        break;
    case E_WP:
        name = "E_WP";
        desc = "Write Protect";
    ...
    case E_BTYP:
        name = "E_BTYP";
        desc = "Bad Type (incompatible) media";
        break;
    case E_DEVBSY:
        name = "E_DEVBSY";
        desc = "Device Busy";
    ...
    case E_LOCK:
        name = "E_LOCK";
        desc = "Record is busy (locked out)";
        break;
    case E_SHARE:
        name = "E_SHARE";
        desc = "Non-sharable file busy";
        break;
    case E_DEADLK:
        name = "E_DEADLK";
        desc = "I/O Deadlock error";
        break;
    case E_FORMAT:
        name = "E_FORMAT";
        desc = "Device is format protected";
        break;
    default:
        name = "E_???";
