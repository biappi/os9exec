...
struct _sgs {
     ...
     */
    unsigned char _sgs_class, /* file manager class code */
    ...
    char _sgs_bspch, /* backspace character */
    ...
    unsigned char _sgs_parity,  /* device initialization (parity) */
        _sgs_baud;              /* baud rate */
    unsigned short _sgs_d2p;    /* offset to second device name string */
    char           _sgs_xon,    /* x-on char */
        _sgs_xoff,              /* x-off char */
        _sgs_tabcr;             /* tab character */
    unsigned char  _sgs_tabsiz; /* tab size */
    unsigned long  _sgs_tbl;    /* Device table address (copy) */
    unsigned short _sgs_col;    /* Current column number */
    unsigned char  _sgs_err;    /* most recent error status */
 ...
 */
struct _sgr {
    unsigned char _sgr_class,  /* file manager class code */
        _sgr_drive,            /* drive number */
        _sgr_step,             /* step rate */
        _sgr_dtype,            /* device type */
        _sgr_dense,            /* density capability */
        _sgr_fill1;            /* not used */
    unsigned short _sgr_cyls;  /* number of cylinders (tracks) */
    unsigned char  _sgr_sides, /* number of sides */
        _sgr_verify;           /* 0 = verify on writes */
    unsigned short _sgr_spt,   /* default sectors per track */
        _sgr_spt0,             /* ditto track 0 */
        _sgr_salloc;           /* segment allocation size */
    unsigned char _sgr_intlv,  /* sector interleave factor */
        _sgr_DMAtfm,           /* DMA transfer mode */
        _sgr_toffs,            /* track offset values */
        _sgr_soffs;            /* Sector offset value */
    unsigned short _sgr_ssize, /* sector size (bytes) */
        _sgr_cntl;             /* device control word */
    unsigned char
        _sgr_trys, /* number of tries counter (1=no-retry/correction) */
        _sgr_lun;  /* SCSI LUN of device */
    unsigned short _sgr_wpc,        /* start write-precompensation cylinder */
        _sgr_rwr,                   /* start reduced write current cylinder */
        _sgr_park;                  /* parking cylinder */
    unsigned long  _sgr_lsnoffs;    /* LSN offset for partition */
    unsigned short _sgr_totcyls;    /* total cylinders on device */
    unsigned char  _sgr_ctrlrid;    /* SCSI controller ID */
    char           _sgr_res2;       /* reserved */
    unsigned       _sgr_scsiopt,    /* SCSI driver options flags */
        _sgr_maxcnt;                /* maximum byte-count passable to driver */
    unsigned      _sgr_partsiz;     /* partition size */
    char          _sgr_rsvd[1];     /* reserved for future use */
    unsigned char _sgr_att;         /* file attributes */
    unsigned long _sgr_fdpsn,       /* file descriptor PSN */
        _sgr_dipsn,                 /* file's directory PSN */
        _sgr_dirptr,                /* directory entry pointer */
        _sgr_dvt;                   /* address of device table entry */
    char          _sgr_resrvd[2];   /* Reserved */
    unsigned long _sgr_sctsiz;      /* sector size */
    char          _sgr_resrvd2[20], /* Reserved */
...
struct _sgsk {
    ...
    union {
        struct {
            unsigned char _sge_devn[32];   /* device name		*/
            char          _sge_enpres[60], /* reserved			*/
                _sge_fname[32];            /* socket name		*/
        } _enpsk;
        struct {
            unsigned short _sgsk_mbpad;    /* mbuf pad value	*/
            char           _sgsk_res[90];  /* reserved space	*/
            unsigned char  _sgsk_devn[32]; /* device name		*/
        } _sock;
    } _sgsk_var;
};
