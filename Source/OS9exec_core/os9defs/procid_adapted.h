...
typedef struct pdsc {
    unsigned short _id,    /* process id */
        _pid,              /* parent's id */
        _sid,              /* sibling's id */
        _cid;              /* child's id */
    unsigned char *_sp,    /* system stack ptr */
        *_usp,             /* user stack ptr */
        *_pagcnt;          /* memory size */
    unsigned short _group, /* group number */
        _user,             /* user number */
        _prior,            /* priority */
        _age,              /* age */
        _state,            /* status */
...
#define ES_FPUCALM040 0x04      /* 68040 fpu exception calmed */
    ...
    char           _dio[DEFIOSIZE];  /* default i/o data */
    unsigned short _path[NUMPATHS];  /* i/o path table */
    unsigned char *_memimg[MEMBLKS]; /* allocated memory block ptrs */
    unsigned int   _blksiz[MEMBLKS]; /* size of each allocated memory block */
    unsigned char *_dbgreg;          /* debugged process register stack frame */
    struct pdsc   *_dbgpar;          /* debugged process parent pd */
    unsigned int   _dbgins,          /* debug instruction count */
    ...
    unsigned short _ioqp,            /* I/O queue prev ptr */
        _ioqn;                       /* I/O queue next ptr */
    unsigned char *_frag, *_fragg;   /* free memory fragment ptrs */
    unsigned int   _sched;           /* process scheduling constant */
    unsigned char *_spuimg;          /* ptr to process SPU image */
    unsigned int   _bkptcnt;         /* number of breakpoints set */
    unsigned short _bpvalue[16];     /* breakpoint save area */
    unsigned int   _acct[8];         /* reserved for user accounting module */
    unsigned char *_data;            /* ptr to process primary data area */
    unsigned int   _datasz;          /* size of primary data area */
    unsigned char *FPUsave;          /* ptr to FPU save area */
    unsigned char *FPExcpt[7],       /* FPU exception vectors */
        *FPExStk[7];                 /* FPU exception stack frame ptrs */
    unsigned char _siglvl,           /* signal interrupt level */
        _sigflg;                     /* signal flag */
    unsigned short _sigxs;    /* number of excess signal blocks allocated */
    unsigned int   _sigmask,  /* mask to disable signals 2 - 31 */
        _sigcnt;              /* number of signals pending */
    unsigned char *_sigque,   /* ptr to head of signal queue */
        _defsig[16],          /* default initial signal queue element */
        *_thread[2],          /* doubly linked thread queue */
        *_cfrag[2];           /* new-style mem color lists */
    unsigned int   _mown;     /* original owner of primary module (security) */
    unsigned char *_expstk;   /* reserved for more stack space */
    unsigned int   _expstksz; /* reserved for more stack space (size) */
    unsigned int   _preempt;  /* process level system-state preemption flag */
    unsigned short _procsiz,  /* size of (complete) process descriptor */
        _stacksiz;            /* size of process stack area */
    unsigned int _sigiret;    /* signal intercept recursion counter */
    unsigned int prcid_res[18]; /* reserved */
