...
#define TIMEOUT 100
    ...
    ii = 0;
    do {
        oserr = OpenDriver(nOut, &mco->rOut);
        if (oserr != 0)
...
OSErr WriteToSerial(ttydev_typ *mco, Ptr theData, Size *lengthOfData)
{
    while (mco->holdScreen)
        CheckInputBuffers(); /* XOn/XOff flow control */
...
/* write characters to console */
{
    ...
    while (n > 0) { /* ignore it if in combination */
        ...
        if (WriteToSerial(mco, tmp, &ii) != noErr)
            return -1;

        if (writtenLF) {
            lf = '\n'; /* add a line feed after CR */
            if (WriteToSerial(mco, &lf, &a) != noErr)
                return -1;
        }
...
void CheckInBufferSerial(int serialID)
{
    ...
    if (cnt == 0)
        return; /* currently nothing to read from serial line */
