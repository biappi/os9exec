...
void handle_os9exec_exception(int nr, uaecptr oldpc)
{
    ...
    if (nr == 32) {
        ...
        put_long(m68k_areg(regs, 7),
                 oldpc + 2); /* +2: is very important (bfo)   */
        // Save vector Offset
        m68k_areg(regs, 7) -= 2;
        ...
        m68k_areg(regs, 5) =
            m68k_areg(regs, 7);             // A5 is pointer to register stack
        m68k_areg(regs, 1) = callers_stack; // A1 is caller's stack
        m68k_dreg(regs, 7) = nr * 4;        // D7 is vector OFFSET
        // now let os9exec_nt do the rest (set A6/PC according to installed
        // error trap handler)
        os9_running = 0; // force exit from emulator loop
    };
}
...
OSErr lowlevel_prepare(void)
{
    ...
    debugprintf(
        dbgStartup,
        dbgDeep,
        ("# lowlevel_prepare: Starting initialisation of UAE 68k emulator\n"));
    /* init UAE emulator */
    savestate_wanted = 0;
